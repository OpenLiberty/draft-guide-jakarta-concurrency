...
The starting application is implemented the [hotspot=getProperties file=1]`/properties/{prefix}` endpoint. Go to the http://localhost:9080/api/system/properties/java URL to see all the `java.*` properties. Notice that the URL takes several seconds to get the result. Each [hotspot=doSomethingSleep hotspot=getSystemPropertyTask hotspot=callGetSystemPropertyTask file=2]`getSystemPropertyTask()` call takes at most a second.

The https://openliberty.io/docs/latest/reference/feature/concurrent-3.1.html[Jakarta Concurrency^] feature is enabled for you in the Liberty [hotspot=concurrent file=0]`server.xml` configuration file.
