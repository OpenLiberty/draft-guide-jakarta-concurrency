...
Use the [hotspot=virtualManagedExecutor hotspot=submit file=4]`virtualManagedExecutor` instance to submit callable tasks that call the `getSystemPropertyTask()` method to get different property values parallelly. Each task creates a virtual thread and returns a `Future<String>` object.
