...
Perform a [hotspot=collect file=4]`collect()` mutable reduction operation on the entries of the [hotspot=properties file=4]`properties` map. In the operation, use the [hotspot=collect file=4]`get()` method to wait for all threads to complete and to collect the results.
