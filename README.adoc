...
The application in this guide provides GET REST APIs to retrieve the Java system properties parallelly and calculate the system load asynchronously. Also, you can toggle the schedule to update the system load.

You'll learn how to run tasks in parallel by enhancing the GET `/api/system/properties/{prefix}` endpoint, create asynchronous task by the GET `/api/system/systemLoad/{after}` endpoint that updates the system load after the provided `{after}` seconds, and schedule a task to update the system load for every 10 seconds.
...
Check out the web application at the http://localhost:9080 URL. Click the <img src="https://raw.githubusercontent.com/OpenLiberty/draft-guide-jakarta-concurrency/blob/gkwan-ibm-jee11/finish/src/main/webapp/img/refresh.svg" width="18" height="18" alt="refresh icon"> refresh button, located in the table title, to update and display the latest system load data in the table after 5 seconds.

Click the <img src="https://raw.githubusercontent.com/OpenLiberty/draft-guide-jakarta-concurrency/blob/gkwan-ibm-jee11/finish/src/main/webapp/img/schedule.svg" width="18" height="18" alt="schedule icon"> schedule button to toggle the schedule that posts the latest system load data in every 10 seconds.
