...
Because the GET [hotspot=getSystemLoad file=2]`/api/system/systemLoad/{after}` endpoint is provided for you, try out the http://localhost:9080/api/system/systemLoad/5 URL. The request returns to you immediatedly with a message.

To see the system load, check out the web application at the http://localhost:9080 URL. Click the image:refresh.svg[refresh icon, 20, 20] refresh button. The latest system load data will be displayed in the table after 5 seconds.
